...
*/
IEnumerable<string> RemoveAnagrams(IEnumerable<string> strings) {
...
List<List<string>> Anagrams = new List<List<string>>() {
    new List<string> { "bac", "acb", "def", "fed", "dfe", "edf", "ghi", "hgi", "ihg" },
    new List<string> { "ibntr","xeuzj","rjnuq","hnfts","vdgny","nuqjr","jeuzx","sldkd","kdjgl" },
    new List<string> { "uuzdwqb","ovzpuqb","rfydzld","wskxxhe","vocmfkt","zqmpffq","ovzpubq","uudzwqb","vockfmt" }
};


foreach (var list in Anagrams) {
    var noAnagram = RemoveAnagrams(list);
    foreach(var str in noAnagram) {
        Console.Write($"{str} ");
    }
    Console.WriteLine();
}
